...
run_full80_eval="${RUN_FULL80_EVAL:-false}"
run_testing_eval="${RUN_TESTING_EVAL:-false}"
...
for ((iteration=start_iteration; iteration<=end_iteration; iteration++)); do
    ...
    validation_manifest="${snapshots_dir}/iter${iteration}_validation_tasks.tsv"

    if [[ ! -f "${validation_manifest}" ]]; then
        python3 tools/rule_curriculum.py write-validation-manifest \
            --task-bank "${task_bank}" \
            --state "${state}" \
            --output "${validation_manifest}"
    fi

    run_validation "${iteration}" "${validation_manifest}"
    if [[ "${run_full80_eval}" == "true" ]]; then run_full80 "${iteration}"; else echo "[rule curriculum] skip Full-80 evaluation"; fi
    if [[ "${run_testing_eval}" == "true" ]]; then run_testing "${iteration}"; else echo "[rule curriculum] skip held-out testing"; fi
