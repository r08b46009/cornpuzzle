...
{
    if (first_piece_of_layer_ || current_layer_row_ < 0 ||
        current_layer_row_ >= active_rows_ || active_cols_ <= 0) {
        return -1;
    }

    // Continue from the right of the previous anchor, wrapping only across
    // active columns.  Occupied cells are skipped; the first empty cell is
    // the deterministic target for the next trimmed piece.
    for (int offset = 0; offset < active_cols_; ++offset) {
        int c = wrapColActive(layer_next_scan_col_ + offset, active_cols_);
        if (board_[current_layer_row_ * kCornPuzzleCols + c] == 0) {
            return current_layer_row_ * kCornPuzzleCols + c;
        }
    }
    return -1;
}
