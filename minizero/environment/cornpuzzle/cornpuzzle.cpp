...
static void loadPieceLibraryFromTxt(const std::string& path)
{
    ...
    if (static_cast<int>(kBasePieces.size()) > kCornPuzzleMaxCandidateShapes) {
        std::cerr << "[CornPuzzle] Too many piece types in library: "
                  << kBasePieces.size()
                  << ". Max supported = "
                  << kCornPuzzleMaxCandidateShapes
                  << std::endl;
        std::exit(1);
    }
...
static CornPuzzleSpec loadPuzzleSpecFromTxt(const std::string& path)
{
    ...
    if (static_cast<int>(grouped.size()) > kCornPuzzleMaxCandidateShapes) {
        std::cerr << "[CornPuzzle] Too many unique candidate shapes in puzzle: "
                  << grouped.size()
                  << ". Max supported = " << kCornPuzzleMaxCandidateShapes
                  << std::endl;
        std::exit(1);
    }
...
void initialize()
{
    ...
    for (int p = 0; p < kCornPuzzleNumPieces; ++p) {
        for (int rot_idx = 0; rot_idx < kCornPuzzleNumRotations; ++rot_idx) {
            ...
            if (action_id == kCornPuzzleReservedActionID) {
                name << "RESERVED";
            } else if (action_id == kCornPuzzleRemoveActionID) {
                name << "REMOVE";
            } else {
                name << "P" << (p + 1) << "R" << rot;
            }
...
void CornPuzzleEnv::resetFromPuzzleFile(const std::string& puzzle_path)
{
    ...
    placement_id_board_.fill(-1);
    placed_pieces_.clear();
    ...
    total_reward_ = 0.0f;
    remove_count_ = 0;
    terminated_by_remove_limit_ = false;
    resetStateVisitMemory();
}
...
bool CornPuzzleEnv::applySolutionPrefix(const std::string& solution_path, int prefix_piece_count)
{
    ...
    for (int label = 1; label <= limit; ++label) {
        ...
        registerPlacedCells(candidate, solution_cells, false);
...
void CornPuzzleEnv::registerPlacedCells(
    int piece_id,
    const std::vector<std::pair<int, int>>& absolute_cells,
    bool removable)
{
    CornPlacedPiece placed;
    placed.placement_id = next_placement_id_++;
    placed.piece_id = piece_id;
    placed.removable = removable;

    for (auto [r, c] : absolute_cells) {
        if (r < 0 || r >= active_rows_ || c < 0 || c >= active_cols_) {
            continue;
        }
        const int pos = r * kCornPuzzleCols + c;
        board_[pos] = piece_id + 1;
        placement_id_board_[pos] = placed.placement_id;
        placed.positions.push_back(pos);
    }

    placed_pieces_.push_back(std::move(placed));
}
...
void CornPuzzleEnv::place(const CornPlacement& placement)
{
    CornPlacedPiece placed;
    placed.placement_id = next_placement_id_++;
    placed.piece_id = placement.piece_id;
    placed.rotation = placement.rotation;
    placed.top = placement.top;
    placed.left = placement.left;
    placed.removable = true;

    const int mark = placement.piece_id + 1;
    for (auto [dr, dc] : placement.cells) {
        const int r = placement.top + dr;
        const int c = wrapColActive(placement.left + dc, active_cols_);
        const int pos = r * kCornPuzzleCols + c;
        board_[pos] = mark;
        placement_id_board_[pos] = placed.placement_id;
        placed.positions.push_back(pos);
...
bool CornPuzzleEnv::isRemovablePosition(int position) const
{
    if (position < 0 || position >= kCornPuzzlePlayableArea) {
        return false;
    }
    const int r = position / kCornPuzzleCols;
    const int c = position % kCornPuzzleCols;
    if (r >= active_rows_ || c >= active_cols_) {
        return false;
    }

    const int placement_id = placement_id_board_[position];
    if (placement_id < 0 || placement_id >= static_cast<int>(placed_pieces_.size())) {
        return false;
    }
    const auto& placed = placed_pieces_[placement_id];
    return placed.active && placed.removable;
}

bool CornPuzzleEnv::hasRemovablePlacement() const
{
    for (const auto& placed : placed_pieces_) {
        if (placed.active && placed.removable) {
            return true;
        }
    }
    return false;
...
bool CornPuzzleEnv::removePlacementAtPosition(int position, int& removed_area)
{
    removed_area = 0;
    if (!isRemovablePosition(position)) {
        return false;
    }

    const int placement_id = placement_id_board_[position];
    auto& placed = placed_pieces_[placement_id];
    for (const int pos : placed.positions) {
        if (pos >= 0 && pos < kCornPuzzleBoardSize &&
            placement_id_board_[pos] == placement_id) {
            board_[pos] = 0;
            placement_id_board_[pos] = -1;
            ++removed_area;
        }
    }

    if (placed.piece_id >= 0 && placed.piece_id < kCornPuzzleNumPieces) {
        ++remaining_[placed.piece_id];
    }
    placed.active = false;
...
std::string CornPuzzleEnv::makeStateVisitKey() const
{
    std::ostringstream oss;

    oss << "rows=" << active_rows_
        << "|cols=" << active_cols_;
...
void CornPuzzleEnv::resetStateVisitMemory()
{
    state_visit_count_.clear();
    terminated_by_loop_ = false;
    state_visit_count_[makeStateVisitKey()] = 1;
}

void CornPuzzleEnv::addClampedRewardToTotal()
{
    const float next_total = total_reward_ + reward_;
    const float clamped_total = std::clamp(next_total,
                                          kCornPuzzleMinEpisodeReturn,
                                          kCornPuzzleMaxEpisodeReturn);
    reward_ = clamped_total - total_reward_;
    total_reward_ = clamped_total;
}

float CornPuzzleEnv::updateStateVisitMemory()
{
    if (terminated_by_loop_) {
        return 0.0f;
    }

    int& count = state_visit_count_[makeStateVisitKey()];
    ++count;
...
bool CornPuzzleEnv::makeActionPlacement(int action_id, CornPlacement& placement) const
{
    if (action_id < 0 || action_id >= static_cast<int>(kCornPuzzleActions.size()) ||
        action_id == kCornPuzzleReservedActionID ||
        action_id == kCornPuzzleRemoveActionID) {
        return false;
    }
...
bool CornPuzzleEnv::act(const CornPuzzleAction& action)
{
    ...
    // REMOVE is a two-step reversible decision using the existing 128 logits:
    //   SELECT_PIECE action 127 -> SELECT_REMOVE_TARGET -> board-cell action.
    if (action_phase_ == CornActionPhase::kSelectPiece &&
        action.getActionID() == kCornPuzzleRemoveActionID) {
        action_phase_ = CornActionPhase::kSelectRemoveTarget;
        pending_piece_id_ = -1;
        pending_rotation_ = 0;
        reward_ = 0.0f;
    ...
    if (action_phase_ == CornActionPhase::kSelectRemoveTarget) {
        int removed_area = 0;
        if (!removePlacementAtPosition(action.getActionID(), removed_area)) {
            return false;
        }

        const float progress_loss =
            static_cast<float>(removed_area) /
            static_cast<float>(std::max(1, activeArea()));
        ++remove_count_;
        reward_ = -0.5f * progress_loss - kCornPuzzleRemoveCost;
        reward_ += updateStateVisitMemory();
        if (terminated_by_loop_) {
            reward_ = kCornPuzzleMinEpisodeReturn;
            total_reward_ = kCornPuzzleMinEpisodeReturn;
        } else if (remove_count_ > kCornPuzzleMaxRemovesPerEpisode) {
            terminated_by_remove_limit_ = true;
            reward_ = kCornPuzzleMinEpisodeReturn;
            total_reward_ = kCornPuzzleMinEpisodeReturn;
        } else {
            addClampedRewardToTotal();
        }
        actions_.push_back(action);
        turn_ = Player::kPlayer1;
        return true;
    ...
    // Finish one complete placement.
    //
    // Important:
    // For a forced RightTrim continuation, the current action is already the
    // meaningful piece/rotation decision. The position is deterministic, so
    // it is applied inside the environment instead of becoming another MCTS
    // node.
    auto commit_placement = [&](const CornPlacement& resolved,
                                int placed_position) -> bool {
        ...
        // Record the action that actually contained decision information.
        //
        // - free first placement: this is the SELECT_POSITION action
        // - forced continuation: this is the SELECT_PIECE action
        actions_.push_back(action);
        ...
        float progress_before =
            static_cast<float>(filled_before) /
            static_cast<float>(std::max(1, activeArea()));

        float progress_after =
            static_cast<float>(filled_after) /
            static_cast<float>(std::max(1, activeArea()));
        ...
        bool dead_end =
            (!solved && (area_mismatch || no_legal_actions));
        ...
        reward_ += updateStateVisitMemory();
        bool loop_terminated = terminated_by_loop_;
        ...
        bool return_floor_terminated =
            total_reward_ <= kCornPuzzleMinEpisodeReturn;

        static int terminal_print_count = 0;
        static int terminal_board_count = 0;

        if (solved || dead_end || loop_terminated || return_floor_terminated) {
            ...
            if (terminal_print_count < 50) {
                std::cerr
                    ...
                    << ", return_floor_terminated=" << return_floor_terminated
            ...
            if (false && terminal_board_count % 10000 == 0) {
                std::ofstream debug_log(
                    cornPuzzleBoardLogPath(),
                    std::ios::app);
    ...
    if (action_phase_ == CornActionPhase::kSelectPiece) {
        ...
        // NEW PART:
        //
        // If this is NOT the first piece of a layer, RightTrim has
        // already determined the position.
        //
        // Therefore:
        //   piece action
        //       -> resolve forced target
        //       -> place immediately
        //       -> next piece decision
        //
        // No SELECT_POSITION MCTS node.
        // ---------------------------------------------------------
        if (!first_piece_of_layer_) {
            const int forced_target =
                getForcedLayerTargetPosition();
        ...
        // ---------------------------------------------------------
        // First piece of a new layer:
        //
        // Position is genuinely free, therefore KEEP Two-Stage.
        // ---------------------------------------------------------
        pending_piece_id_ = selected.piece_id;
        pending_rotation_ = selected.rotation;

        action_phase_ =
            CornActionPhase::kSelectPosition;

        reward_ = 0.0f;

        // Piece choice is still a real search action.
        actions_.push_back(action);
    ...
    // -------------------------------------------------------------
    // SELECT_POSITION now happens only for the first piece of a layer.
    // -------------------------------------------------------------
...
bool CornPuzzleEnv::isLegalAction(const CornPuzzleAction& action) const
{
    ...
    if (terminated_by_remove_limit_) {
        return false;
    }

    if (total_reward_ <= kCornPuzzleMinEpisodeReturn) {
        return false;
    }
    ...
    if (action_phase_ == CornActionPhase::kSelectPiece) {
        if (action_id == kCornPuzzleRemoveActionID) {
            return hasRemovablePlacement();
        }
        if (action_id == kCornPuzzleReservedActionID) {
            return false;
        }
    ...
    if (action_phase_ == CornActionPhase::kSelectRemoveTarget) {
        return action_id < kCornPuzzlePlayableArea &&
               isRemovablePosition(action_id);
    }
...
std::vector<float> CornPuzzleEnv::getFeatures(utils::Rotation rotation) const
{
    ...
    for (int p = 0; p < kCornPuzzleNumPieces; ++p) {
        ...
        // The legacy final candidate plane (P64) is reserved as a REMOVE-mode
        // target mask. This keeps the input tensor exactly 67 x 14 x 14.
        if (p == kCornPuzzleNumPieces - 1 &&
            action_phase_ == CornActionPhase::kSelectRemoveTarget) {
            for (int pos = 0; pos < kCornPuzzleBoardSize; ++pos) {
                plane[pos] = isRemovablePosition(pos) ? 1.0f : 0.0f;
            }
        } else if (p < static_cast<int>(candidate_shapes_.size()) && remaining_[p] > 0 &&
...
std::vector<float> CornPuzzleEnv::getActionFeatures(const CornPuzzleAction& action, utils::Rotation rotation) const
{
    ...
    if (action_phase_ == CornActionPhase::kSelectPiece &&
        action.getActionID() == kCornPuzzleRemoveActionID) {
        for (int pos = 0; pos < kCornPuzzleBoardSize; ++pos) {
            action_features[pos] = isRemovablePosition(pos) ? 1.0f : 0.0f;
        }
        return action_features;
    }

    if (action_phase_ == CornActionPhase::kSelectRemoveTarget) {
        const int pos = action.getActionID();
        if (isRemovablePosition(pos)) {
            const int placement_id = placement_id_board_[pos];
            const auto& placed = placed_pieces_[placement_id];
            for (const int cell_pos : placed.positions) {
                if (cell_pos >= 0 && cell_pos < kCornPuzzleBoardSize) {
                    action_features[cell_pos] = 1.0f;
                }
            }
        }
        return action_features;
...
std::string CornPuzzleEnv::actionToConsoleString(const CornPuzzleAction& action) const
{
    ...
    if (position < 0 || position >= kCornPuzzlePlayableArea) {
        return action_phase_ == CornActionPhase::kSelectRemoveTarget ? "RM?" : "L?";
    }

    std::ostringstream oss;
    oss << (action_phase_ == CornActionPhase::kSelectRemoveTarget ? "RM" : "L")
...
std::string CornPuzzleEnv::toString() const
{
    ...
    if (action_phase_ == CornActionPhase::kSelectPosition) {
        oss << "phase=SELECT_POSITION pending=P" << (pending_piece_id_ + 1)
            << "R" << pending_rotation_ << "\n";
    } else if (action_phase_ == CornActionPhase::kSelectRemoveTarget) {
        oss << "phase=SELECT_REMOVE_TARGET\n";
    } else {
        oss << "phase=SELECT_PIECE\n";
    }
